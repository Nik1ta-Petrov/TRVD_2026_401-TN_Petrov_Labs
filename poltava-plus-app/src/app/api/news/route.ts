...
export async function GET(request: Request) {
    // Отримуємо ліміт із параметрів запиту (наприклад, ?limit=20)
    const { searchParams } = new URL(request.url);
    const limit = searchParams.get("limit") || "6";

    try {
        // Вставляємо змінну limit у запит до WordPress
        const response = await fetch(
            `https://np.pl.ua/wp-json/wp/v2/posts?per_page=${limit}&_embed`,
            {
                cache: "no-store",
                headers: {
